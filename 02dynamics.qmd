...
Mechanics is the branch of physics concerned with the motion of bodies and the forces that influence it. This chapter begins with kinematics, the study of motion without reference to its causes, where we describe position, velocity, and acceleration in one and two dimensions. Building on this foundation, we then explore dynamics, examining situations where forces produce acceleration, and present methods for analyzing the resulting motion, including the application of Newton’s laws, energy principles, and momentum conservation.
...
::: {#exm-ex2-2}
A ship's engines are stopped when traveling at a speed of 18 knots, and the ship comes to rest after 20 min. Assuming uniform deceleration, find the deceleration (m/s²) and the distance traveled in nautical miles in that time.
...
print(f"Distance traveled: {s_nmi:.3f} nautical miles")
```
:::
...
::: {#exm-ex2-6}
A ship’s propellers are stopped when traveling at 25 knots. From that point, it travels 4 kilometers before coming to a complete stop. Calculate the time it takes to stop in minutes and the average deceleration in m/s².
...
::: {#exm-ships}
Two ships are moving toward each other in still water. Ship 1 is 310 meters long and traveling at 22 knots, while Ship 2 is 350 meters long and traveling at 19 knots. Calculate the time it takes for the ships to completely pass each other..
